...
def import_prog(ctrl_dir, ctrl_name, prog_name):
    """
    Dynamically import a Python module and retrieve a specific attribute.

    This function enables runtime import of Python modules generated by p4toz3,
    allowing the semantic analyzer to load and execute the translated P4 code.

    Args:
        ctrl_dir (Path): Directory containing the Python module to import
        ctrl_name (str): Name of the Python module (without .py extension)
        prog_name (str): Name of the attribute/class to retrieve from the module

    Returns:
        object: The requested attribute/class from the imported module
    """
...
def get_z3_asts(p4_module, p4_path):
    """
    Execute P4 Python module to generate Z3 abstract syntax trees.

    This function instantiates the P4 program with Z3 context, converting the
    Python representation into formal Z3 logical formulas that can be analyzed
    mathematically.

    Args:
        p4_module: Python module object containing the translated P4 program
        p4_path (Path): Original P4 file path for logging and error reporting

    Returns:
        tuple: (z3_asts, exit_code) where z3_asts contains the Z3 representations
               and exit_code indicates success (EXIT_SUCCESS), failure (EXIT_FAILURE),
               or skip (EXIT_SKIPPED) if no main module found
    """
...
def get_py_module(prog_path):
    """
    Load the Python module containing the translated P4 program.

    This function imports the Python file generated by p4toz3 and extracts
    the main P4 program function for semantic analysis.

    Args:
        prog_path (Path): Path to the Python file containing translated P4 code

    Returns:
        object or None: The P4 program module object, or None if import fails
    """
...
def run_p4_to_py(p4_file, py_file):
    """
    Execute p4toz3 to translate P4 program to Python+Z3 representation.

    This function runs the p4toz3 compiler extension to convert a P4 source
    file into Python code that generates Z3 logical formulas for semantic analysis.

    Args:
        p4_file (Path): Input P4 source file to translate
        py_file (Path): Output Python file path for the translated code

    Returns:
        subprocess.CompletedProcess: Process execution result with return code,
...
def get_z3_formulization(p4_file, out_dir=OUT_DIR):
    """
    Convert P4 program to Z3 formal representation.

    This function orchestrates the complete pipeline from P4 source to Z3 formulas:
    1. Translate P4 to Python+Z3 (if input is .p4 file)
    2. Load the Python module
    3. Execute to generate Z3 abstract syntax trees

    Args:
        p4_file (Path): Input file (P4 source or Python+Z3 code)
        out_dir (Path): Output directory for intermediate files

    Returns:
        tuple: (package, exit_code) where package contains the Z3 representation
               and exit_code indicates success or failure
    """
